...
/**
* Kaltura HTML5 Library v1.6.12.13 
...
window['SCRIPT_LOADER_URL']='http://cdnakmi.kaltura.org/apis/html5versions/1.6.12.12/ResourceLoader.php';window['KALTURA_LOADER_VERSION']='1.6.12.13';(function(){"use strict";var kWidget={readyWidgets:{},readyCallbacks:[],setup:function(){var _this=this;this.checkEnvironment();this.overrideFlashEmbedMethods();this.proxyJsCallbackready();this.domReady(function(){_this.proxyJsCallbackready();});this.domReady(function(){_this.rewriteObjectTags();});},checkEnvironment:function(){if(document.URL.indexOf('forceMobileHTML5')!==-1&&!mw.getConfig('disableForceMobileHTML5')){mw.setConfig('forceMobileHTML5',true);}
...
_this.jsCallbackReady(widgetId);}}},jsCallbackReady:function(widgetId){if(typeof this.proxiedJsCallback=='function'){this.proxiedJsCallback(widgetId);}
...
this.jsReadyCalledForIds=[];},embed:function(targetId,settings){if(typeof targetId==='object'){settings=targetId;if(!settings.targetId){this.log('Error: Missing target element Id');}
...
break;}}
if(settings.isHTML5){this.outputHTML5Iframe(targetId,settings);}else{this.outputFlashObject(targetId,settings);}},embedFromObjects:function(rewriteObjects){for(var i=0;i<rewriteObjects.length;i++){var settings=rewriteObjects[i].kEmbedSettings;settings.width=rewriteObjects[i].width;settings.height=rewriteObjects[i].height;this.embed(rewriteObjects[i].id,rewriteObjects[i].kEmbedSettings);}},outputFlashObject:function(targetId,settings){var elm=document.getElementById(targetId);if(!elm&&!elm.parentNode){kWidget.log("Error embed target missing");return;}
...
options.width=400;if(!settings.wid&&settings.partner_id){settings.wid='_'+settings.partner_id;}
if(!settings.partner_id&&settings.wid){settings.partner_id=settings.wid.replace('_','');}
var baseUrl=SCRIPT_LOADER_URL.replace('ResourceLoader.php','');var downloadUrl=baseUrl+'download.php/wid/'+settings.wid;if(settings.uiconf_id){downloadUrl+='/uiconf_id/'+settings.uiconf_id;}
if(settings.entry_id){downloadUrl+='/entry_id/'+settings.entry_id;}
var thumbSrc=this.getKalturaThumbUrl({'entry_id':settings.entry_id,'partner_id':settings.partner_id,'height':(document.body.clientHeight)?document.body.clientHeight:'300','width':(document.body.clientHeight)?document.body.clientHeight:'400'});var playButtonUrl=baseUrl+'skins/common/images/player_big_play_button.png';var playButtonCss='background: url(\''+playButtonUrl+'\'); width: 70px; height: 53px; position: absolute; top:50%; left:50%; margin: -26px 0 0 -35px;';var ddId=(settings.id)?settings.id:'dd_'+Math.round(Math.random()*1000);var ddHTML='<div id="'+ddId+'" style="width: '+options.width+';height:'+options.height+';position:relative">'+'<img style="width:100%;height:100%" src="'+thumbSrc+'" >'+'<a id="directFileLinkButton" href="'+downloadUrl+'" target="_blank" style="'+playButtonCss+'"></a>'+'</div>';var parentNode=targetNode.parentNode;var div=document.createElement('div');div.style.width=options.width+'px';div.style.height=options.height+'px';div.innerHTML=ddHTML;parentNode.replaceChild(div,targetNode);if(!document.getElementById(ddId)){parentNode.insertBefore(div,targetNode);}},addReadyCallback:function(readyCallback){for(var widgetId in this.readyWidgets){if(document.getElementById(widgetId)){readyCallback(widgetId);}}
